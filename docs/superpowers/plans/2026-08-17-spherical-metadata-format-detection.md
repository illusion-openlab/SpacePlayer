...
- `sv3d` box: presence alone (not its internal `proj`/`equi` contents) is the signal used — maps to `Projection.SPHERE_360`. **Correction (found during the final whole-branch review, verified against real file bytes):** this is a deliberate scope decision, not because "no box-level signal for `HEMISPHERE_180` exists" — `sv3d/proj/equi` (`EquirectangularProjectionBox`) defines `projection_bounds_top/bottom/left/right`, and non-zero crop bounds are the standard way 180°-vs-360° is actually expressed; a real file (`360.mp4`) was confirmed to carry this exact sub-structure (bounds all zero here, i.e. genuinely uncropped). Because this plan only checks `sv3d` presence and never reads `proj`/`equi`, any real file that uses non-zero crop bounds to express 180° content will be misdetected as full `SPHERE_360` — a confident wrong answer, not a graceful "not detected." A file with `st3d` but no `sv3d` at all still just leaves projection undetected (defaults to `FLAT`, same as any other "not detected" case) — that part is accurate. See `AGENTS.md`'s 2026-08-17 entry for the full finding.

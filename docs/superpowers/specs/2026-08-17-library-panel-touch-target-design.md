...
- **Environment `ToggleableChip`s**: pass `chipSize = ChipsDefaults.Regular` explicitly (40.dp, up from the default `ChipsDefaults.Small` = 32.dp), removing the per-chip `.padding(end = 8.dp)` — see spacing model below.
...
- **Spacing model**: add `horizontalArrangement = Arrangement.spacedBy(16.dp)` to the bar's single outer `Row` (removing every individual `.padding(end = Ndp)` on its children) instead of introducing nested sub-`Row`s per group — `Arrangement.spacedBy` inserts a real gap between *every* pair of adjacent direct children regardless of how many conditionally render, so one Row-level declaration produces the same result nested grouping would, with less code. This also adds a small (harmless) gap on either side of the existing `Spacer(Modifier.weight(1f))` that still splits the left (environment chips) and right (format/subtitle/play) groups — negligible against that Spacer's flexible width, and not a click-adjacency concern since nothing is tightly packed against it.

...
1. **旧的 Google Spatial Media V1 元数据方案（`uuid` box 里嵌 XML 文本）明确不做**。**更正（2026-08-17
   装机验证后）**：设计阶段曾以为验证用的三个真实文件都用新方案（`st3d`/`sv3d`）——这个假设是错的。真机
   验证时逐字节核对了三个文件的实际 box 树，`360_TB.mp4` 实际上完全没有 `st3d`/`sv3d`，它的立体/投影
   信息是通过 track 级 `uuid` box（UUID `ffcc8263-f855-4a93-8814-587a02521fdd`）里的 GSpherical XML
   携带的——这正是本条要跳过的旧方案。也就是说这条"已知限制"不是纸面上的假设情况，用户给的验证素材里
   就真实存在一个会命中它的文件，装完之后这个文件会被当成普通平面单眼视频（默认兜底 FLAT/MONO）。旧方案
   解析作为后续扩展点记录，不在这次范围内。详见 `AGENTS.md` 2026-08-17 记录。
2. **只有 `st3d` 没有 `sv3d` 的文件（比如这次的 `180-LR.mp4`）拿不到 180 这个信息**——立体格式（SBS）
   能正确识别，投影方式因为这次没有解析对应信号会落到默认 FLAT，需要用户手动改成 180。**更正（全量分支
   review 指出，原表述不准确）**：这里的准确说法不是"`sv3d` 规范里没有 180° 的标准化信号"——`sv3d/proj/
   equi`（`EquirectangularProjectionBox`）本身定义了 `projection_bounds_top/bottom/left/right` 四个
   0.32 定点数字段，非零的裁切边界正是标准规定的表达"这是部分球面（比如 180°）而不是完整球面"的方式；
   `360.mp4` 的 `sv3d/proj/equi` 就真实带着这四个字段，实测取值全为零（确认这份素材确实是无裁切的完整
   360°，不是我们没解析导致的误判）。这次的设计选择是**只看 `sv3d` box 是否存在，不解析它内部的 `proj`/
   `equi` 子结构**——所以任何真实存在、且用非零裁切边界表达 180° 的文件，会被这次的实现当成完整 360°
   误判（不是"检测不到"这种保守的兜底，而是"检测到了但答案确定性地错了"），比"没有该信号"这个说法所暗示
   的后果更值得注意。不打算用宽高比之类的启发式去猜——这正是这次要删除宽高比兜底的原因；后续如果要补上
   180° 检测，正确路径是解析 `proj`/`equi` 的裁切边界，而不是重新引入宽高比启发式。

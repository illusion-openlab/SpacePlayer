...
**符号已根据两个真实观察定为负号（`-60f`）**：装机后 `+60f` 面板变成一条线（完全侧对），
而"旋转生效前"的默认朝向是"顶部朝我倒"——侧对 = 偏离正对 90 度，所以默认本身就已经朝用户倾了约 30 度，
正号是继续加大这个倾斜。负号才是朝上翻的方向。没有用能精确凑到"正好上仰 60 度"的 `-90f`：那个值只有在
"默认恰好 30 度"这个估计完全准确时才对，一旦默认更接近 0，就会翻过头重新变成一条线；`-60f` 在这个估计的
整个合理区间里都是清楚可读的。如果现在觉得仰得不够，加大数值是安全方向（往这边退化是平缓的，往侧对退化不是）。
下面这段是当时写的、现在已被上述观察取代的说明：**这个 60 度的正负号未经验证，必须说清楚**：之前所有装机观察对判断符号都是无效证据（那时旋转压根没生效，
...
## 2026-08-18 续三：HUD 倾斜定为 -30 度，以及"捏合点按钮却把面板关掉"的冲突

- **倾斜角**：`+60f` 装机后面板变成一条线（完全侧对）。侧对 = 偏离正对 90 度，而"旋转生效前"看到的默认朝向
  是"顶部朝我倒"，所以默认本身已经朝用户倾了约 30 度，正号是继续加大这个倾斜、负号才是朝上翻。据此定为负号，
  幅度按用户要求取 30：`HUD_PITCH_DEGREES = -30f`。**注意：这两条才是唯一有效的符号证据**——在把赋值挪进帧
  循环之前，旋转根本没生效，那期间所有"方向反了"的反馈描述的都是同一个默认朝向，对判断符号毫无价值。

- **捏合冲突（用户提出，按"第一优先级为控制面板，控制面板没有响应才做显隐"实现）**：PICO 的手部交互本身就是
  "射线 + 捏合 = 点击"，所以全局监听捏合必然和点面板上的按钮撞车——用户想点按钮，结果面板被关掉。
  实现：`PlaybackHud` 根 `Box` 加 `Modifier.hoverable(interactionSource)`，用
  `collectIsHoveredAsState()` 把"指针/射线是否在面板上"通过新增的 `onPointerOverChange` 回调报给
  `ImmersiveScene`，帧循环里的判断改成 `if (pinch.justEngaged && !isHudPointerOver)`。
  于是：指着面板捏 = 按下射线下面那个按钮，显隐不变；指着别处捏 = 切换显隐（顺带也是个不用找按钮的关闭方式）。
  **失败模式是安全的那一侧**：万一这个 hover 信号在真机上压根不来，`isHudPointerOver` 恒为 false，行为退回
  到改动前（捏合照样能呼出/关闭），而不是变成"面板再也叫不出来"。`PlaybackHud` 里还加了 `DisposableEffect`
  在销毁时把这个状态复位，避免残留的 true 永久吞掉呼出手势。
